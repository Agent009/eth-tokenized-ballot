...
async function main() {
    ...
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> token contract address`, token.address);
    ...
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> minted ${MINT_AMOUNT.toString()} decimal units to user 1 account: ${deployerAddress}`);
    // Get balance
    const balanceDeployer = await token.read.balanceOf([deployerAddress]);
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> balance of user 1 ${deployerAddress}: ${balanceDeployer}`);
    // Get the current and target block numbers
    const currentBlockNumber = await publicClient.getBlockNumber();
    const targetBlockNumber = currentBlockNumber + 1n;
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> Current block number`, currentBlockNumber);
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> Target block number`, targetBlockNumber);
    ...
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> tokenized ballot contract address`, ballot.address);
    ...
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> minted ${MINT_AMOUNT.toString()} decimal units to user 2 account: ${acc1Address}`);

    // Getting voting power
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> getting voting power for user 1...`);
    const votingPower = await ballot.read.getVotePower([deployerAddress]);
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> Voting power of user 1 ${deployerAddress}: ${votingPower}`);
    ...
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> delegated voting power to user 1 ${deployerAddress}`);

    // Voting
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> Voting...`);
    ...
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> vote transaction hash: ${votingTx}`);

    const winningProposal = await ballot.read.winningProposal();
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> winning proposal: ${winningProposal}`);

    const winner = await ballot.read.winnerName();
    console.log(`scripts -> ${TOKEN_CONTRACT_NAME} -> TokenizedScript -> winner: ${winner}`);
}
